...
<div class="news-detail">
	...
	<? if ($arParams["DISPLAY_NAME"] != "N" && $arResult["NAME"]): ?>
		...
		<a href="?zal=1&id=<?= $arResult["ID"] ?>" id="zalob"><?= GetMessage('POZALOV') ?></a><br>
...
<?php
if ($arParams['SET_AJAX_ZALOBA'] === 'Y'):?>
	<script>
		let flag = 1;
		BX.bind(BX('zalob'), 'click', function() {
			if (flag){
				flag = 0;
				BX.ajax.post(
					'.',
					...
					function (data) {
						BX('result_zalob').innerHTML = data;
